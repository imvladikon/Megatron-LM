...
from megatron.core.ssm.kda_config_utils import validate_kda_config
...
class KimiDeltaAttention(_GDNBase):
    ...
    def __init__(
        ...
        *,
        ...
        if pg_collection is None or pg_collection.tp is None or pg_collection.cp is None:
            raise ValueError("KDA requires explicit TP and CP process groups.")
        validate_kda_config(config, tp_size=pg_collection.tp.size(), cp_size=pg_collection.cp.size())
        ...
        if self.two_stage_gates:
            # The released Flash checkpoint and HF implementation keep the
            # short convolution in FP32, independently of projection precision.
            mark_keep_in_fp32(self.conv1d.weight)
            if self.conv1d.bias is not None:
                mark_keep_in_fp32(self.conv1d.bias)
            ...
            for prefix, head_dim, output_dim in (
                ...
                setattr(
                    ...
                    build_module(
                        ...
                        # TELinear requires no communication group for a
                        # replicated projection; the following B projection
                        # owns TP communication (including SP gather/scatter).
                        tp_group=None,
    ...
    def _setup_variant_attrs(self) -> None:
        ...
        self.conv_params_dtype = torch.float32 if self.two_stage_gates else self.config.params_dtype
    ...
    def forward(
        ...
        **kwargs,
        ...
        # HybridStack owns transitions and packed metadata through the existing
        # ContextParallelLayoutManager. A layer consumes its declared layout;
        # converting here again would reorder tokens twice.
        ...
        if cp_size_headwise > 1 and self.config.linear_cp_layout != "zigzag":
            raise ValueError("KDA headwise CP requires linear_cp_layout='zigzag'.")
        if cp_size_chunkwise > 1 and self.config.linear_cp_layout != "contiguous":
            raise ValueError("KDA chunkwise CP requires linear_cp_layout='contiguous'.")
        if self.config.deterministic_mode and (packed_seq_params is not None or cp_size_chunkwise > 1):
            raise NotImplementedError(
                "KDA deterministic convolution currently supports unpacked headwise CP only; "
                "a plain conv1d does not reset at packed boundaries or exchange chunkwise state."
            )
        ...
        if cp_size_chunkwise > 1:
            # Optional CP support must not change availability of ordinary GDN
            # or TP-only KDA on FLA installations without this extra API.
            ...
            if cu_seqlens_q is None:
                ...
                if cached is None:
                    ...
                    # One live cached shape/group, independent of run length.
                    # In-flight forwards retain their own context for backward.
                    self._chunkwise_cp_context_cache.clear()
    ...
    def pre_gated_delta_rule(
        ...
        *,
        ...
        if self.config.deterministic_mode or causal_conv1d is None:
            ...
            conv_out = F.conv1d(
                input=qkv.to(conv1d_weight.dtype),
            ...
            qkv = self.act_fn(conv_out[..., :kernel_seq_len]).to(qkv.dtype)
            qkv = qkv.transpose(1, 2).contiguous()
        else:
